...
For spoken answers, press the microphone to start and press **Готово** when finished. The second press stops capture while the browser finishes recognizing the recorded speech. Final results are handled immediately without waiting for the recognition-end event; empty recordings allow a retry without consuming an attempt. Leaving the study or diagnostic screen cancels any active recording.
